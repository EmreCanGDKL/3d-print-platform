...
type DefectKey = 'healthy' | 'warping' | 'stringing' | 'layer_shift' | 'cracking' | 'off_platform';
...
const defectCopy: Record<DefectKey, Omit<DefectResult, 'key' | 'confidence' | 'source'>> = {
  healthy: {
...
function chooseDemoDefect(file: Express.Multer.File): DefectResult {
  const bytes = file.buffer;
  const sampleSize = Math.min(bytes.length, 60_000);
  let total = 0;
  let bright = 0;
  let contrast = 0;
  ...
  let key: DefectKey = 'healthy';
  if (textureScore > 82 && brightRatio > 0.12) key = 'stringing';
  else if (textureScore > 95) key = 'off_platform';
  else if (mean < 82) key = 'layer_shift';
  else if (mean > 160 && textureScore > 55) key = 'cracking';
  else if (brightRatio < 0.035 && mean > 120) key = 'warping';
...
async function analyzeWithModelService(file: Express.Multer.File): Promise<DefectResult | null> {
  if (!modelServiceUrl) return null;
  ...
  const predictedKey = String(response.data?.key || '').trim() as DefectKey;
