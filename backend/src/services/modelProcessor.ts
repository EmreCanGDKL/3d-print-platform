...
function parseSTL(buffer: Buffer): { positions: number[]; vertexCount: number } {
...
function calculateVolumeAndArea(positions: number[]): { volume: number; surfaceArea: number } {
  let volume = 0;
  let surfaceArea = 0;
...
export async function processModelForSecureViewing(modelBuffer: Buffer): Promise<SecureGeometryData> {
  ...
  const secureData: SecureGeometryData = {
    type: 'SecureGeometry',
    version: '1.0',
    metadata: {
      vertexCount,
...
export function getMetadataForSQLite(secureData: SecureGeometryData) {
  return {
