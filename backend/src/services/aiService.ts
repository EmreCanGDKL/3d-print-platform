...
let hitem3dTokenCache: { token: string; expiresAt: number } | null = null;
...
export class AIService {
  private async getHitem3dAccessToken(): Promise<string> {
    ...
    if (hitem3dTokenCache && hitem3dTokenCache.expiresAt - Date.now() > 60_000) {
  ...
  async generateFromImage(
    ...
    if (hasHitem3dCredentials()) {
      try {
        ...
        const response = await axios.post(`${HITEM3D_BASE_URL}/open-api/v1/submit-task`, formData, {
          headers: {
            ...formData.getHeaders(),
  ...
  async checkTaskStatus(taskId: string): Promise<TaskStatusResult> {
    if (hasHitem3dCredentials()) {
      try {
        const accessToken = await this.getHitem3dAccessToken();
        const response = await axios.get(`${HITEM3D_BASE_URL}/open-api/v1/query-task`, {
